...
| [`run_gpu_smoke_diffusion_e2e.sh`](../../tests/gpu_smoke/run_gpu_smoke_diffusion_e2e.sh) | `ci-e2e-diffusion` | 4 | Diffusion end-to-end training (FlowGRPO, online DPO, DiffusionNFT, MiniMax-H3 on the v1 sync trainer; v1 separate_async; v0 remains for Qwen-Image-Edit, Bagel, and the v0 separate layout) |
