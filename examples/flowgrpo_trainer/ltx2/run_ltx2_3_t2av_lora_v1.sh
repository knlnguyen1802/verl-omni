#!/usr/bin/env bash
# LTX-2.3 text-to-audio-video LoRA FlowGRPO recipe (V1 trainer: TransferQueue +
# ReplayBuffer + sync mode).
#
# This is the v1 counterpart of run_ltx2_3_t2av_lora.sh. It uses the new
...
# set on the CLI.
#
# Keep `actor_rollout_ref.rollout.agent.default_agent_loop=ltx2_diffusion_single_turn_agent`
# (custom T2AV loop). Tensor parallel, LoRA, and video+audio knobs match the v0
# GPU recipe.
#
# Reference (legacy v0 script):
# verl-omni/examples/flowgrpo_trainer/ltx2/run_ltx2_3_t2av_lora.sh
set -x
