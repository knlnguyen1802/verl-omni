#!/usr/bin/env bash
# Bagel LoRA OCR recipe (V1 trainer: TransferQueue + ReplayBuffer + sync mode).
#
# This is the v1 counterpart of run_bagel_ocr_lora.sh. It uses the new
...
# set on the CLI.
#
# Keep BAGEL-specific knobs identical to the v0 GPU OCR LoRA recipe:
# architecture, trust_remote_code, deploy yaml, LoRA targets, reward, and data.
#
...
# verl-omni/examples/flowgrpo_trainer/bagel/run_bagel_ocr_lora.sh
set -x
...
python3 -m verl_omni.trainer.main_diffusion_v1 \
    ...
    trainer.experiment_name=bagel_ocr_lora_v1 \
