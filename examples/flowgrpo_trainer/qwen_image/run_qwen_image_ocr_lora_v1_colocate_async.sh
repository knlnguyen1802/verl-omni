#!/usr/bin/env bash
# Qwen-Image LoRA OCR recipe (V1 trainer: colocate_async mode).
#
# This is the colocate-async counterpart of run_qwen_image_ocr_lora_v1.sh. It
# uses `verl_omni.trainer.main_diffusion_v1` with
# `trainer.v1.trainer_mode=colocate_async`, which selects
# `PolicyGradientDiffusionTrainerV1ColocateAsync`. Trainer and rollout share one
# GPU pool: warmup batches keep generation in flight across step boundaries,
# in-flight samples are aborted and slept at sample end, retried as whole
# samples by `DiffusionWholeSampleRetryLLMServerClient`, and rollout weights
# are updated and resumed at step end.
#
# This script is GPU-unverified: it ships ahead of the smoke run so the mode is
# reproducible once hardware is available. Watch `training/rollout_retry/*` and
# `training/off_policy/*` on the first runs.
#
...
# Colocate-async knobs. Warmup batches keep one extra batch of generation in
# flight while the first step waits on its sample.
NUM_WARMUP_BATCHES=${NUM_WARMUP_BATCHES:-1}

# Colocated memory layout (colocated modes, 4x 80GB GPUs): between phases the
# rollout engine sleeps at level 1 — vllm-omni offloads the whole pipeline (transformer
...
python3 -m verl_omni.trainer.main_diffusion_v1 \
    ...
    trainer.experiment_name=qwen_image_ocr_lora_v1_colocate_async \
