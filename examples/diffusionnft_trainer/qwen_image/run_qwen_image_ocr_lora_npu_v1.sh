#!/usr/bin/env bash
# Qwen-Image DiffusionNFT LoRA RL on Ascend NPU (V1 trainer: TransferQueue + ReplayBuffer + sync mode).
#
# This is the v1 counterpart of run_qwen_image_ocr_lora_npu.sh. It uses
...
# set on the CLI. Attention / TP layout stays on the v0 NPU script.
#
# Reference (legacy v0 script):
# verl-omni/examples/diffusionnft_trainer/qwen_image/run_qwen_image_ocr_lora_npu.sh
set -x
...
python3 -m verl_omni.trainer.main_diffusion_v1 \
    ...
    trainer.experiment_name=qwen_image_ocr_lora_npu_v1 \
